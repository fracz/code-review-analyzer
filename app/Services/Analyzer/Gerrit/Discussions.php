...
class Discussions extends AbstractAnalyzer
{
	use StringTitle;
	...
	{
            //echo "echo from Discussions";exit;
            ...
            foreach ($results as &$result) {
                    $result['messages'] = array_filter($result['messages'], function($item){
                            return count($item['replies']) > 0;
                    });
	...
	private function addRepliesOld($messages, $replies)
	{
            if (empty($messages)) {
                return [];
            }
            ...
            if (!empty($deep)) {
                foreach ($messages as $key => $message) {
                    if (!empty($message['replies'])) {
                        $messages[$key]['replies'] = $this->addRepliesOld($message['replies'], $deep);
                    }
                }
            }
	...
	{
            return view('review.gerrit.discussions._results', ['result' => $result, 'project' => $project]);
	}
}
